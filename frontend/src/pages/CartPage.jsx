...
export default function CartPage() {
  const { items, addItem, decrementItem, removeItem, totalPrice, seat } = useCart();
  const navigate = useNavigate();

  if (!seat) { navigate("/"); return null; }
  ...
  return (
    <div className="min-h-screen cb-grain pb-40">
      <AppHeader title="Your Cart" backTo="/menu" testId="cart-header" />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-3">
                      ...
                      <button
                        ...
                        data-testid={`remove-${it.item_id}`}
                        className="text-white/40 hover:text-[#E50914] transition-colors"
                        aria-label="Remove"
                      >
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center gap-1 rounded-full bg-white/10 border border-white/10">
                        <button
                          onClick={() => decrementItem(it.item_id)}
                          data-testid={`cart-dec-${it.item_id}`}
                        ...
                        <button
                          onClick={() => addItem({ id: it.item_id, name: it.name, price: it.price, image: it.image })}
                          data-testid={`cart-inc-${it.item_id}`}
                          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/10 active:scale-90 transition-all"
                          aria-label="Increase"
                        >
                      ...
                      <span className="font-mono text-sm" data-testid={`cart-line-${it.item_id}`}>
